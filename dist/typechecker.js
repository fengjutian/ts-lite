import { TypeErrorItem, ANY, NUMBER, STRING, BOOLEAN } from './schema.ts';
function typeFromNode(node, typeParamNames) {
    if (!node)
        return ANY;
    if (node.kind === "Name") {
        if (node.name === "number")
...
function substituteGenerics(t, map) {
    if (!t)
        return ANY;
    if (t.tag === "var")
...
export class TypeChecker {
    errors = [];
    globals = new Map(); // function / value types globally
    checkProgram(prog) {
    ...
    checkNode(node, env) {
        switch (node.type) {
            ...
            case "FuncDecl": {
                ...
                const fnType = { tag: "func", params: paramTypes, ret: retType, typeParams: node.typeParams };
