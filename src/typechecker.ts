import type { ASTNode, TypeNode, Type, Loc } from './types.js';
...
export function typeFromNode(node?: TypeNode, typeParamNames?: Set<string>): Type {
  if (!node) return ANY;
  if (node.kind === "Name") {
...
// 使用映射替换泛型类型变量
export function substituteGenerics(t: Type, map: Map<string, Type>): Type {
  if (!t) return ANY;
  if (t.tag === "var") 
...
export function isCompatible(a: Type, b: Type): boolean {
  if (!a || !b) return false;
...
export class TypeChecker {
  ...
  globals = new Map<string, Type>(); // 全局函数/值类型
